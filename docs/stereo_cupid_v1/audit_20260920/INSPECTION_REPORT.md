...
**16:20更新：官方权重下载CPU305505已恢复，GPU305510等待下载成功。** 旧305474因SSH失效/Xet停滞已取消，305481亦取消且无模型执行。下载采用4MiB持久化Range并保留完整官方SHA检查，未安装依赖；完整校验尚未完成。最新路径、失败记录和跟进方式见 `../RUN_STATUS.md`。下文关于下载禁令和待授权的问题属于历史状态，已按用户澄清更正为仅禁止登录节点大型资产下载。
