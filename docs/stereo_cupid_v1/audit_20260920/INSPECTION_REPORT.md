...
**15:52更新：用户已允许官方权重下载。** CPU305474正在server14通过已安装官方HF/Xet客户端下载固定版本，完整校验尚未完成；305446是依赖失败后取消的0秒GPU作业，没有真实模型执行。最新路径、失败记录和跟进方式见 `../RUN_STATUS.md`。下文关于下载禁令和待授权的问题属于历史状态，已被用户最新授权取代。
