...
20:33当前入口覆盖下方旧记录：CPU306009/server14 RUNNING，GPU306014等待；checkouts3f9d24f/a22、权重a12复用a11、输出STEREO_CUPID_PILOT_3F9D24F_A8。转发exec5028，节点loopback49694→本机实测20890；下载终态关闭。现有路线已实际新增分块，不因为7999/直连探测失败停止它。临时网络错误保留10分钟恢复窗口；完整性错误仍5次，Slurm2h及请求超时不变。若故障先恢复实际端口与既有认证通道，健康作业不重提。用户提出本地落盘中转作为备选，仅1MiB小样测试通过，不能称已在本机下载整套权重。
