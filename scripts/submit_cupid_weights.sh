...
download_args=(--output "$CUPID_MODEL_PATH" --wait-local-proxy "${CUPID_WAIT_LOCAL_PROXY_SECONDS:-0}" --client "${CUPID_DOWNLOAD_CLIENT:-requests}")
if [[ -n "${CUPID_REUSE_WEIGHTS_FROM:-}" ]]; then download_args+=(--reuse-from "$CUPID_REUSE_WEIGHTS_FROM"); fi
"$runtime/bin/python3" -u scripts/download_cupid_weights.py "${download_args[@]}"
